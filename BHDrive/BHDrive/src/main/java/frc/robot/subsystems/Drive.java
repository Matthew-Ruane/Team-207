...
import frc.utility.BHRDifferentialDrive;
import frc.utility.BHRMathUtils;
import frc.utility.Loop;
import frc.utility.MPSoftwarePIDController;
import frc.utility.MPSoftwarePIDController.MPSoftwareTurnType;
import frc.utility.MPTalonPIDController;
import frc.utility.PIDParams;
import frc.utility.SoftwarePIDController;
import frc.utility.TalonSRXEncoder;
import frc.utility.TalonSRXFactory;
import frc.utility.control.Kinematics;
import frc.utility.control.Lookahead;
import frc.utility.control.Path;
import frc.utility.control.PathFollower;
import frc.utility.control.RobotState;
import frc.utility.math.RigidTransform2d;
import frc.utility.math.Rotation2d;
import frc.utility.math.Twist2d;
...
//            System.out.println("vel Com u/s = " + command + ", vel com in/sec= " + left_inches_per_sec * scale + ", scale = " + scale + ", left pos in = " + getLeftPositionInches()  + ", right pos in = " + getRightPositionInches() + ", left vel in/sec = " + getLeftVelocityInchesPerSec() + ", left vel u/s = " + leftDrive1.getSelectedSensorVelocity(0));
    ...
    public synchronized void driveWithJoystick() {
		...
		m_moveInput = OI.getInstance().getDriverController().getRightY();
		m_steerInput = -OI.getInstance().getDriverController().getLeftX();
