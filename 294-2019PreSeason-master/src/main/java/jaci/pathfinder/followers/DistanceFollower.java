...
public class DistanceFollower {
    ...
    double dt;
    double last_error, heading, startTimeMillis, currentTimeMillis, segment_offset;
    ...
    public double calculate(double distance_covered) {
        ...
        segment_offset = (currentTimeMillis - startTimeMillis)/1000/dt;
        segment = (int) Math.floor(segment_offset);     // closest segment index that we just went past
        segment_offset = segment_offset - segment;      // fractional offset to next segment

        if (segment < trajectory.length()-1) {
            Trajectory.Segment seg = trajectory.get(segment);           // Get the last segment
            if (segment >= trajectory.length()-1) {
                position = seg.position;
                velocity = seg.velocity;
            } else {
                Trajectory.Segment seg_next = trajectory.get(segment+1);    // Get the next segment

                // interpolate the position and velocity that we should be at right now
                position = seg.position + (seg_next.position - seg.position)*segment_offset;
                velocity = seg.velocity + (seg_next.velocity - seg.velocity)*segment_offset;
            }

            double error = position - distance_covered;
            double calculated_value =
                    kp * error +                                    // Proportional
                    kd * ((error - last_error) / dt) +               // Derivative
                    (kv * velocity + ka * seg.acceleration);        // V and A Terms
            last_error = error;
            heading = seg.heading;
